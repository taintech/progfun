...
object Huffman {
   ...
   *   }
   */
  def times(chars: List[Char]): List[(Char, Int)] = chars.groupBy(c => c).mapValues(_.length).toList
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    table.find(kv => kv._1 == char).map(_._2).getOrElse(List())
  ...
  def convert(tree: CodeTree): CodeTable = {
    @tailrec
    def loop(loopChars: List[Char], agg: CodeTable): CodeTable = loopChars match {
      case Nil => agg
      case c::cs => loop(cs, (c,encode(tree)(List(c)))::agg)
    }
    loop(chars(tree), List.empty).reverse
  }
